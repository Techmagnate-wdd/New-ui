...
import { getExcel, getFeaturedSnippetDetails, getProjects, getSerpCategoryOnlySummaryDashboard, getSerpCategorySubCategorySummaryDashboard, getSerpCategorySummaryDashboard, getSerpSummaryDashboard, getUnifiedRankingDashboard } from "../../services/api";
...
export default function SerpCategoryOnlySummaryDashboard() {
  // filters + selections
  ...
  const fetchFeaturedStatusesForPage = useCallback(async (selectedProjectId, pageNum, limit, appliedFilters = {}) => {
    ...
    try {
      ...
      const resp = await getSerpCategoryOnlySummaryDashboard(
  ...
  const tableData = useMemo(() => {
    if (!usingExcel) {
      return (pagedRows || []).map((r, idx) => ({
        ...
        count: r.keywordCount ?? r.keywordCount ?? "",
