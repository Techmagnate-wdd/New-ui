...
import { getExcel, getFeaturedSnippetDetails, getProjects, getSerpCategorySummaryDashboard, getSerpSummaryDashboard, getUnifiedRankingDashboard } from "../../services/api";
...
export default function SerpCategorySummaryDashboard() {
  // filters + selections
  ...
  const fetchFeaturedStatusesForPage = useCallback(async (selectedProjectId, pageNum, limit, appliedFilters = {}) => {
    ...
    try {
      ...
      const resp = await getSerpCategorySummaryDashboard(
  ...
  const tableData = useMemo(() => {
    if (!usingExcel) {
      return (pagedRows || []).map((r, idx) => ({
        ...
        brandType: r.brandType ?? r.brandType ?? "",
