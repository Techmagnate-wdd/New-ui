...
import { getLLMProjects, getLLMPromptsWithUrls, getLLMThirdPartyPages } from "../../services/api";
import { useLocation, useNavigate } from "react-router-dom";
import * as XLSX from 'xlsx';
import TooltipIcon from "./ToolTipIcon";
import AuthContext from "../../context/AuthContext";

const PLATFORMS = ["ChatGPT", "Perplexity", "Gemini", "Claude", "ai_Mode"];
...
const ThirdPartyPages = () => {
    ...
    const fetchCitations = async (projectId, page = 1) => {
        ...
        try {
            ...
            const response = await getLLMThirdPartyPages({
                filters,
                page,
                limit: pageSize
            });
            ...
            if (response?.data?.data) {
                ...
                const projectCompetitors = (project.competitors || []).map((c) => ({
                    brand: c.brand,
                    domain: c.domain?.toLowerCase(),
                }));

                // Merge main project + competitors into one list
                const allCompetitors = [mainProject, ...projectCompetitors];

                // Map: domain -> brand
                const competitorMapObj = allCompetitors.reduce((acc, competitor) => {
                    if (!competitor?.domain) return acc;
                    acc[competitor.domain] = competitor.brand;
                    return acc;
                }, {});

                // Save into state
                setAvailableCompetitors(allCompetitors);   // array of objects
                setCompetitorMap(competitorMapObj);        // { domain: brand }
            }
        } catch (err) {
    ...
    // Fetch data when project, page, or filters change
    useEffect(() => {
        if (projectId) {
    ...
    // Get project ID from URL and fetch competitors
    useEffect(() => {
        const pathParts = location.pathname.split("/");
        const idFromUrl = pathParts[pathParts.length - 2];
        setProjectId(idFromUrl);
        // if (idFromUrl) {
        //     fetchAllCompetitors(idFromUrl);
        // }
    }, [location.pathname]);
    ...
    //     const updated = selectedCompetitors.includes(competitor)
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const worksheetData = [
                ['Your Prompt', 'Cited Pages', 'Source Type', 'Platform'],
                ...allData.map(row => [
                    row.prompt || '',
                    row.url || '',
                    row.source_type || '',
                    LLM_NAME_MAP[row.platform] || '',
                    row.date ? new Date(row.createdAt).toLocaleDateString() : ''
                ])
            ];
    ...
    const handleSelectCompetitor = (domain) => {
        setSelectedCompetitors((prev) =>
            prev[0] === domain ? [] : [domain]      // click again to clear
        );
        setCurrentPage(1);
    };
                        ...
                        {loading ? (
                            <>
                                <div className="col-md-3">
                                    <div className="shimmer" style={{ height: "38px", borderRadius: "4px" }}></div>
                                </div>
                            ...
                            <>
                                ...
                                <div className="col-md-2" ref={competitorDropdownRef} style={{ position: "relative" }}>
                                    ...
                                    >
                                        {selectedCompetitors.length > 0
                                            ? toTitleCase(
                                                competitorMap[selectedCompetitors[0]] || selectedCompetitors[0]
                                            )
                                            : "Competitor"}
                                    </button>
                                        ...
                                        >
                                            {availableCompetitors.length > 0 ? (
                                                availableCompetitors.map((competitor) => (
                                                    <div key={competitor.domain} className="form-check mb-2 d-flex">
                                                        <div className="d-flex gap-2 align-items-center">
                                                            <input
                                                                className="form-check-input"
                                                                type="radio"                      // <--- radio here
                                                                name="competitor"                // same name => radio group
                                                                value={competitor.domain}
                                                                id={`competitor-${competitor.domain}`}
                                                                checked={selectedCompetitors[0] === competitor.domain}
                                                                onChange={() => handleSelectCompetitor(competitor.domain)}
                                                            />
                                                        </div>
                                                        <label
                                                            className="form-check-label"
                                                            htmlFor={`competitor-${competitor.domain}`}
                                                        >
                                                            {competitor.brand}
                                                        </label>
                                                    </div>
                                ...
                                {/* Clear Filters */}
                                {(selectedPlatforms.length > 0 || selectedCompetitors.length > 0 || searchPrompt || selectedDate) && (
                                    <div className="col-md-2">
                                        <button
                    ...
                    <div className="d-flex align-items-center justify-content-between mb-16">
                        <div style={{ color: "#000", fontWeight: 500, fontSize: "1.5rem" }}>
                            {loading ? (
                                ...
                                <div className="shimmer" style={{ height: "24px", width: "180px", borderRadius: "4px", marginBottom: "4px" }} />
                            ) : (
                                <>
                                    Domains mentioning me
                                    <p style={{ fontSize: "12px", color: "#8b8e98" }}>
                                        Domains that mention your brand on their webpage and are being cited as sources in AI answers
                                    </p>
                                </>
                            )}
                        </div>
                        {loading ? (
                            ...
                            <div className="shimmer" style={{ height: "36px", width: "100px", borderRadius: "6px" }} />
                        ) : (
                            <button
                                style={{
                                    backgroundColor: "#fff",
                            ...
                            <thead className="table-light" style={{ position: "sticky", top: 0, zIndex: 1 }}>
                                <tr className="text-nowrap">
                                    ...
                                    <th scope="col">
                                        <span>
                                            Source Type
                                            < TooltipIcon title={"Identifies whether the cited domain belongs to your brand, or a third party."} />
                                        </span>
                                    </th>
                                    <th scope="col">
                                        <span>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    Array.from({ length: 5 }).map((_, idx) => (
                                        <tr key={idx}>
                                            <td><div className="shimmer" style={{ height: "20px", width: "90%", borderRadius: "4px" }} /></td>
                                            <td><div className="shimmer" style={{ height: "20px", width: "80%", borderRadius: "4px" }} /></td>
                                            <td><div className="shimmer" style={{ height: "20px", width: "70%", borderRadius: "4px" }} /></td>
                                            <td><div className="shimmer" style={{ height: "20px", width: "60%", borderRadius: "4px" }} /></td>
                                        </tr>
                                    ))
                                    ...
                                    filteredData.map((r, i) => (
                                        <tr key={`${r.taskId}-${i}`} className="text-nowrap">
                                            <td title={r.prompt} style={{ cursor: "pointer" }}>
                                                {r.prompt
                                                    ? r.prompt.length > 100
                                            ...
                                            <td>{r.source_type || ""}</td>
                                            <td style={{ textTransform: "capitalize" }}>{LLM_NAME_MAP[r.platform] || ""}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
