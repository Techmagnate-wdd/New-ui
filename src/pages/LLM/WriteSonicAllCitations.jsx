...
const WriteSonicAllCitations = ({ selectedProject }) => {
    ...
    const fetchCitations = async (projectId, page = 1) => {
        ...
        try {
            ...
            if (response?.data?.data) {
                ...
                // Extract unique competitors from domains
                // const competitors = new Set();
                // rows.forEach(row => {
                //     if (row.domain) {
                //         const brand = extractBrandFromDomain(row.domain);
                //         if (brand) competitors.add(brand);
                //     }
                // });
    ...
    // Fetch initial data to get all unique competitors
    const fetchAllCompetitors = async (projectId) => {
        if (!projectId) return;
        ...
        try {
            ...
            const response = await getLLMPromptsWithUrls({
                filters,
                page: 1,
                limit: 10 // Get more rows to extract unique competitors
            });
...
export default WriteSonicAllCitations;