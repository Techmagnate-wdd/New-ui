...
import { getShopping, getVideos } from "../../services/api";
...
const ShoppingCard = ({ filter, setTotalSV, setTotalKeywords, setSelectedProjectData }) => {
    ...
    const fetchSerpFeatures = useCallback(
        async (currentFilter, signal) => {
            ...
            try {
                const resp = await getShopping(query, { signal });
    ...
    const loadDetailsPage = useCallback(
        async (pageNum, pageSize, ctxOverride) => {
            ...
            try {
                ...
                const resp = await getShopping(query, {
                ...
                const rows = list.map((it) => ({
                    ...
                    displayed_price: it.displayed_price || null,
                    title: it.title || null,
                    url: it.url || null,
                    rank: it.rank || null,
                    sv: it.searchVolume || 0,
                }));

                console.log(rows,"rows first")
        ...
        <div className="mt-4">
            {loading ? (
                ...
                >
                    ...
                    >
                        <h6 style={{ color: "#4a4a4a", margin: 0 }}>Shopping</h6>
