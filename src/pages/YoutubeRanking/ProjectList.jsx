...
import { blockUser, deleteLLMProjects, deleteUser, deleteYoutubeProjects, editLLMProjects, editYoutubeProjects, getLLMProjects, getUsers, getYoutubeProjects } from "../../services/api";
...
import EditYoutubeProject from "./EditYoutubeProject";

const YoutubeProjectList = () => {
  ...
  const columns = [
    ...
    { title: "Channel ID", dataIndex: "target", key: "target" },
  ...
  const fetchProjects = async (filters) => {
    try {
      ...
      const response = await getYoutubeProjects(filteredQuery);
  ...
  const handleProjectSubmit = async (payload) => {
    try {
      await editYoutubeProjects(editingProject._id, payload);
  ...
  const confirmDelete = async (projectId) => {
    try {
      await deleteYoutubeProjects(projectId);
    ...
    <div className="project-dashboard container1 py-4">
      ...
      >
        ...
        <div style={{ backgroundColor: "#487fff", borderRadius: "4px" }}>
          <button
            onClick={() => navigate("/upload-youtube-rank")}
            style={{
...
export default YoutubeProjectList;
