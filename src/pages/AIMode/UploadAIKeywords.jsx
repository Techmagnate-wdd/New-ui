...
const UploadAIKeywords = () => {
  ...
  // when targetId arrives, fire off the report creation
  useEffect(() => {
    if (!targetId) return;

    (async () => {
      try {
        await createAIModeReport({ project_id: targetId });
      } catch (err) {
        console.error("Failed to create AI mode report:", err);
      }
    })();
  }, [targetId]);
...
// ------------------------------------created by awasthi ai-----------------
 const handleCompetitorChange = (index, field, value) => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append("device", formData.device_type);
    ...
    try {
      const resp = await UploadAIModeKeywords(data);
      console.log(data,"tigerai")
...
export default UploadAIKeywords;
